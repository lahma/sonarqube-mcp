...
| D17 | **NuGet is a second distribution channel, not the primary one.** `dotnet pack` produces `sonarcloud-mcp`, a framework-dependent .NET tool package (`PackAsTool`) carrying `PackageType=McpServer` and an embedded `.mcp/server.json`, so `dnx sonarcloud-mcp@{version}` works without a download step; the Native AOT binaries stay the recommended way to run the server. **The NuGet package id is the one name that is not `sonarqube-mcp`** — 2026-08-22, on the first push of v1.0.0, nuget.org rejected the id because SonarSource holds the reserved `SonarQube*` id prefix. Only `<PackageId>`, `.mcp/server.json`'s `identifier` and its NuGet entry, the plugin's `dnx` argument and the prose that quotes them moved; `AssemblyName`, `ToolCommandName`, the MCP server `name`, the plugin and marketplace names, the archive names and the namespaces are all still `sonarqube-mcp`, so `dnx sonarcloud-mcp` installs and runs a command called `sonarqube-mcp`. Do not "fix" the mismatch by renaming anything else: the id a package is fetched by is not the name it runs under, and the reservation is not going away. It is pushed by **trusted publishing** — the workflow exchanges its GitHub OIDC token for an API key that lives minutes — so no NuGet API key exists in the repository or in GitHub secrets. The exchange is **C# inside the build** (`build/Build.Publish.cs`), not a marketplace action. It has its own generated workflow file, `.github/workflows/publish.yml`, triggered by `v*.*.*` tags only. |
...
The same manifest bundles the MCP server (`dnx sonarcloud-mcp@{version} --yes` — the NuGet id, D17,
