...
// Deviation from the Cake original: the upstream file throws Microsoft.Build.Exceptions.BuildAbortedException.
// Fallout only carries Microsoft.Build.Locator at runtime (real MSBuild is resolved lazily), so touching
// that type from OnBuildInitialized fails with "Could not load file or assembly 'Microsoft.Build'".
...
public sealed class ReleaseNotesParser
{
    ...
    public IReadOnlyList<ReleaseNotes> Parse(string content)
    {
        ...
        throw new InvalidOperationException("Unknown release notes format.");
    }
    ...
    private IReadOnlyList<ReleaseNotes> ParseComplexFormat(string[] lines)
    {
        ...
        while (true)
        {
            ...
            if (!version)
            {
                throw new InvalidOperationException("Could not parse version from release notes header.");
            }
